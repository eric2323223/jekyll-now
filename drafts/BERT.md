...
为了对token进行分类判断，需要在BERT的输出上增加一个多类型分类器（在实现中被称为MLM head，包含一个全连接网络和softmax计算），应用这个分类器对所有token计算出每个类型的概率，再和这个token的真实分类进行比较，计算误差。
##### Mask LM 误差函数
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTQ0OTk4OTMzNSw0NTY3Nzc5NTAsMjAwNz
