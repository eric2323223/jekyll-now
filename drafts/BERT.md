...
$$ L_{mlm}(\theta, \theta_{mlm}) = -\sum_{i=1}^M\log p(m=m_i| \theta, \theta_m)$$
其中  $\theta$表示BERT预训练模型的参数， $\theta_{mlm}$表示MLM任务层（多类型分类器）的参数，$M$表示
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTg1MTM1NTM1NywtMTMyNjM4OTY0MSwxNj
