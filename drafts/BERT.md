...
当把vocabulary中所有的词都作为一个单独的类型时，对未知token进行预测就xiang   为了对token进行分类判断，需要在BERT的输出上增加一个多类型分类器（在实现中被称为MLM head），它包含一个全连接网络和softmax运算，可以将通过BERT 编码器编码过的token转换为vocabulary长度个输出，每个输出代表属于对应分类的概率。应用这个分类器对所有token计算出每个类型的概率，再和这个token的真实分类进行比较，通过cross entropy函数计算误差。之所以对全部token进行分类预测的原因是由于被遮罩的词有15%的机率被替换成随机词，因此每个词都可能是被遮罩过的。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE5NDAwNjAyOTksLTE2OTM1NzI2MTEsLT
