...
##### MLM 误差函数
MLM对每个~~被遮罩的~~词进行预测（由于因为不知道哪个词是被遮罩的），判断预测值和真实值的过程是典型的分类问题（每个不同的词都是一个类型），因此MLM的误差函数使用分类问题常用的crossentropy函数
```
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTU1MDM2MzIxNiwtMTAzODY1NTgyNyw4OT
