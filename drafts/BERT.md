...
为了对token进行分类判断，需要在BERT的输出上增加一个多类型分类器（在实现中被称为MLM head），应用这个分类器对所有token进行，通过softmax运算计算出每个分类的概率，在和这个token的真实分类进行比较，计算误差。
##### Mask LM 误差函数
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE3NzI4OTgzMTIsNDU2Nzc3OTUwLDIwMD
