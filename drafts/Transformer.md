...
#### 自注意力（self attention）
时序问题（特备是NLP问题）中的序列元素表示的意义通常不止该单个元素的的意义，而是与整个序列上下文有关系，因此在encoding过程中需要考虑整个序列来决定其中每个元素的意义。self-attention作用于每一个序列元素，简单来说它使用整个序列所有元素的**加权**平均来确定该元素的含义。其中的权值来自该元素与其他元素的相似度，由于通常使用点积运算。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNDc5NDY2ODk4LDU4MTEyMzI1OSwtMzc3OD
