...
下图可以发现，RNN需要在依次处理元素x1, x2和x3之后才能得到整个序列的上下文信息，而attention则可以同时处理x1，x2，x3而得到序列的上下文信息。导致的结果context vector的形式由原来RNN的一个固定长度的context vector变成了可变长度（和输入长度相同）的
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTQwNjUzODY3OCwxNDczNzY3MTgwLDE4NT
