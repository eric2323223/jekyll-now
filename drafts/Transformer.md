...
由于attention机制不考虑位置关系，因此必须要在在attention操作前对序列中的每个元素加入位置信息。一个最直接的方法就是对输入加入序号，但是这种方法的问题在于无法处理长度超过训练数据的输入序列。在Transformer模型中使用的是sin/cos函数进行位置编码，这种位置编码有两个优点，首相就是由于sin/cos函数的周期性它能够进行任意长度序列的位置编码，解决了之前的问题。第二是由于sin/cos函数的值
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbOTEyNTcyNjk0LDYxODUxMjY5MiwtMjcxNT
Q1NTcsMTYyODU0NDAxMSwxMzg2NTI4NTUyLC0xMzYwMjQwMDU4
LC0yMDA0NDE4ODMwLC0yMDE2NjIxMDI3LC05MTI1ODU3NjQsMj
A3NTc2MjA5LDE1MTA4ODU0MzEsMjA1MjM5MTc5MCwtMTMyNDMw
NjI3MCwtMTM1NDI2NDg5MSwxNTYzNjExMTc4LDExNjMwMDA5ND
AsLTYzMTgzMDM5OCwxMjIyMDUwMDgsLTM1MDQzNzU0NCwtOTg5
NDUzOTA4XX0=
-->