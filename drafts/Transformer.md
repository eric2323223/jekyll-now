...
前面我们介绍了注意力机制
前面介绍过使用编码器-解码器架构处理时序任务的基本思路，其中编码器的基本任务是分析处理输入序列并生成序列编码（context vector），基于RNN模型的方法是利用RNN的存储能力，在编码阶段通过不断的处理输入元素并更新内部状态，逐渐积累输入元素所包含的信息从而最终获得这个输入序列的上下文信息（context vector）。Transformer利用注意力机制
生成
