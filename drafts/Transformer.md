...
和卷积神经网络通过使用多个卷积核来发掘不同特征的思路类似，多头注意力也是通过多次随机初始化过程来提取不同特征，具体来说是对同一个元素进行多次注意力运算， 每次注意力计算之前都使用随机生成的$W^Q, W^K, W^V$来得到$Q,K,V$，最后再将多次运算的结果相加。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTI1OTg0MDkyNCw1MDE3MzMwMjgsODM2OD
