...
标签平滑归一化通过"软化"传统的独热one-hot类型编码，使得在计算误差值时能够有效抑制过拟合现象。它的实现非常简单，通过一个超参数$\epsilon$如下图所示，label smoothing相当于减少真实样本标签的类别在计算损失函数时的权重，最终起到抑制过拟合的效果。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwMjIxNjg1ODMsLTUzNTUyNDQxNCwxNj
