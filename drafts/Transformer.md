...
解码器需要两个输入来生成输出，第一个输入是上一步的解码器输出（第一个解码器输出由一个固定的标识编码充当），这个输入首先要通过位置编码器加入位置信息，在通过解码器的带遮罩的自注意力MHA（图中Masked Multi-Head Attention）加入上下文信息到已输出元素，之后进行编码器-解码器MHA加入序列编码sequence embedding中的特征信息，最后在经过按位
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyMDc5NzA3ODcsMTA5NDc4NTkxNiw0Nz
UyMTMxOCwtNjA2NTIxMzAwLDE0OTI0NDQ0NDYsLTEzMjAyNDE0
MjAsMTE5NDk4OTcyMSwyNzAzODQ1MDEsLTE3OTU0Njg1NiwxMD
QxOTM0MDc5LC0xMzg1MTUyODkyLC03NTg2MjcxNzgsNzAzODQw
OTU4LC0xODcwNDkwNjQzLDEzMTMzNjU2MzYsMTE1OTMwNzk5Ni
wtOTQxMjI1MzY2LDY2NjQ1MTkyMiw0Mjc4MTY2NjgsMTM4MjQ1
NjIwNV19
-->