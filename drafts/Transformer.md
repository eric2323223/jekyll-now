...
和卷积神经网络通过使用多个卷积核来发掘不同特征的思路类似，多头注意力也是通过多次随机初始化过程来提取不同特征，具体来说是对同一个元素进行多次注意力运算， 每次注意力计算之前都使用随机生成的参数$W$来得到$Q,K,V$（具体如下1,2,3），再将多次运算的结果合并。合并的过程是串联（concatenate）并通过和$W^O$进行矩阵相乘得到和输入同样尺寸的结果。
$$XW^Q=Q$$
$$XW^K=K$$
$$XW^V=V$$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTIzNzU0NzA4NCw1MDE3MzMwMjgsODM2OD
