...
- Position encoding
由于transformer不使用RNN和CNN，仅仅计算不同元素之间的相似度，因此必须加入位置信息来保证transformer正确的理解输入序列。最简单的位置信息是输入元素的序号，但这种方式对输入序列的长度敏感，当
- Attention 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbODkwMTkxOTA1LDkwNzMxOTkzOCwyMTM2Nz
