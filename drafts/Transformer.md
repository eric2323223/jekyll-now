...
标签平滑归一化通过"软化"传统的独热one-hot类型编码，使得在计算误差值时能够有效抑制过拟合现象。它的实现非常简单，通过一个超参数$\epsilon$将原来的0，1分布变成$\epsilon, 1-\epsilon$分布（对于二值分类问题），这样就可以缩短真假值之间的距离，最终起到抑制过拟合的效果。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTU2NDY1MzI1NSwtNTM1NTI0NDE0LDE2OT
