...
前面介绍过使用编码器-解码器架构处理时序任务的基本思路，其中编码器的基本任务是分析处理输入序列并生成序列编码（context vector），基于RNN模型的方法是利用RNN的存储能力，在编码阶段通过不断的处理输入元素并更新内部状态，逐渐积累输入元素所包含的信息从而最终获得这个输入序列的上下文信息（context vector），同样在解码阶段根据context vector产生输出。在transformer模型中设计了自注意力机制来生成conext vector
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE0MDcyNTE3NTQsMTk2OTQ1OTYxNiwxNT
k2NDQwNTQwLDk2MDcxMDMzNiwtNzU1NzQ4MzM4LC00MjgzNzUw
NDAsMTY5MzQzNTIxNSwxMTIwMDk3OTYyLC0yMzcxNzI2ODUsMT
Q4OTc3NzM3NywtMTQyMDYwMjAzOCwxOTE1MzUwMzY4LC0xMjkw
NDM5MzYxLDY0Mjk0MjIyLC0xNTMxMzIyMjA0LDIxMTY3MDc2OD
MsODQ1MzI3MDcxLDIxMjI0ODgzODIsMTU3MDMyMTEyOCwtMjE0
NjU4NDQ0NF19
-->