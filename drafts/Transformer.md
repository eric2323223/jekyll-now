...
从实现上来讲，attention操作可以理解为加权求和的运算，加数是序列中的所有元素，权值计算方法根据任务目标而不同（在机器翻译的场景中使用相似度来作为权值）。用$\alpha$表示权值（通常是概率，即$\sum \alpha$），$h$表示序列元素，可以将attention形式化的表示为
$$c_i=\sum_{j=1}\alpha_{ij}h_j$$
从这个定义可以看出attention的结果$c$就是序列中所有元素按一定的比例关系
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwODA5MjY4MDIsLTEzNjAyNDAwNTgsLT
