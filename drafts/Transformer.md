...
从实现上来讲，attention操作可以理解为加权求和的运算，加数是序列中的所有元素，权值计算方法根据任务目标而不同（在机器翻译的场景中使用相似度来作为权值）。用$\alpha$表示权值（通常是概率，即$\sum \alpha=1$），$h$表示序列元素，可以将attention形式化的表示为
$$c_i=\sum_{j=1}\alpha_{ij}h_j$$
从这个定义可以看出attention的结果$c$就是序列中所有元素按一定的比例关系相加得到的，由于$c$具备了序列的上下文信息，因此我们也可以把attention理解为**元素在某一个序列上下文环境中的重新定义**。这是attention最核心的特点，也是attention能够取代RNN的基础。下图可以发现，RNN
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNTIwMDYwMTMwLC0xMzYwMjQwMDU4LC0yMD
A0NDE4ODMwLC0yMDE2NjIxMDI3LC05MTI1ODU3NjQsMjA3NTc2
MjA5LDE1MTA4ODU0MzEsMjA1MjM5MTc5MCwtMTMyNDMwNjI3MC
wtMTM1NDI2NDg5MSwxNTYzNjExMTc4LDExNjMwMDA5NDAsLTYz
MTgzMDM5OCwxMjIyMDUwMDgsLTM1MDQzNzU0NCwtOTg5NDUzOT
A4LC0xNDY3MjExNjc2LDE2MDM1ODA2MjUsLTI3MTU1MzU0NSwx
OTIxMTQwMDk0XX0=
-->