...
基于值的方法的基本思想是求一个函数Q满足bellman方程，使得在状态$s$下使用动作$a$可能得到最大的奖赏
$$
Q(s,a) = r + \gamma max_{a'} Q(s', a')
$$
其中，$s'$表示在当前状态$s$下使用动作$a$得到的下一个状态，$\gamma$表示对未来事件的折扣率，bellman方程描述了一个递归的计算方法，即$s$状态在使用$a$动作得到的总奖励等于当前的直接奖励$r$和下一步动作$a'$产生的总奖励的和。从这个方程可知，为了计算$s$状态下的最大
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTg4OTA0OTI0N119
-->