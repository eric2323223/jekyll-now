...
$$
另外一个思路是通过引入基准(baseline)$b$减小方差，这是因为实际计算中产生的总奖励并不能准确代表这个策略的好坏程度，比如在有可能一个不太好的策略也能得到一个正向的总奖励$J(\theta)$，PG算法依然会由于这个正向的$J(\theta)$调整原来的策略而导致学习效果下降
$$
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbOTg5NzM4NDE0LDE2NTc2MjE3MzldfQ==
-->