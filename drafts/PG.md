...
强化学习的过程这样的，处于初始状态的Agent产生随机的动作作用于环境，环境返回给Agent新的状态和对动作的奖励，这个奖励可以是正向（表示动作的好的）也可以是负向的（表示动作是不好的），根据反馈，Agent通过算法决定
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTc1Mjk5NDUzNSwxNzIzMDk3NjA2LDE5Nz
QxMzAyOThdfQ==
-->