...
$$
其中$\mathbb E[r_0+r_1+r_2+...+r_t|\pi_\theta]$表示在策略$\pi_\theta$条件下一轮交互（$t_0$到$t_t$步）中的累计奖励的期望值，这里是期望而不是确定值是因为每一步的奖励是根据策略得到的期望值而不是确定值。由于$r_i \sim P(s)\pi(a_i | s_{i-1})|s_{i-1})$, 可以把累计奖励的期望值写成如下：
$$
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbODAwMjY3NDY5LC03MDYwOTYyM119
-->