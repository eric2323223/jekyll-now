...
强化学习包括了一系列不同的算法，其中比较常见的是基于值（Value-based）的方法和基于策略（Policy-based）的方法。这两种方法各有特点，适用于解决不同的问题。一般来说，基于值的方法适用于比较简单（状态空间比较小）的问题，它有较高的数据利用率并且能稳定增强学习效果；而方法基于策略的方法适用于复杂问题，但是高方差是方法一个比较明显的问题。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjg5MTk3NDIzLC00NzczMzAzMTcsLTg1OD
