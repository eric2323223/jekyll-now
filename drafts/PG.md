...
-  **𝛾** : 折扣系数，用于调整未来对当前的影响
强化学习的过程是一个通过和环境交互获得反馈，再根据返回调整以期使总奖励最大化的过程，需要强调的是这个是一个多步的交互的过程，每一步交互都会影响其后的所有步骤。强化学习中的交互是指Agent 对环境施加一个动作，这会导致导致环境的状态发生改变并且由环境回馈给Agent一个奖励（ji
### 强化学习的基本目标
是奖励最大话
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE4NjgzNTkwMzcsLTQzNDcyMjk5XX0=
-->