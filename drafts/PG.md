...
强化学习包括了一系列不同的算法，其中比较常见的是基于值（Value-based）的方法和基于策略（Policy-based）的方法。这两种方法各有特点，适用于解决不同的问题。一般来说，基于值的方法适用于比较简单（状态空间比较小）的问题，它有较高的数据利用率并且能稳定增强学习效果；而方法基于策略的方法适用于复杂问题，但是高方差是这类方法一个比较明显的问题。本文介绍的策略梯度就是一种的最基本的基于策略的方法，我们
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTU0NzM1NzcwMiwtNDc3MzMwMzE3LC04NT
