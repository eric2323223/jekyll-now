...
$$
策略：$\pi_\theta(a|s)$ 表示在状态￥s和参数\\thetaθ条件下发生动作aa的概率
...
## PG的目标函数
根据上述PG的基本原理，策略可以表示为$\pi_\theta(a|s)$， 意为在状态$s$和参数$\theta$条件下发生动作$a$的概率。我们可以把PG的目标描述为以下表达式
$$
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1MDA2MTU5MTYsLTcwNjA5NjIzXX0=
-->