...
-  **𝛾** : 折扣系数，用于调整未来对当前的影响
>强化学习其实也是机器学习的一个分支，但是它与我们常见的机器学习（比如监督学习supervised learning）不太一样。它讲究在一系列的情景之下，通过多步恰当的决策来达到一个目标，是一种**序列多步决策**的问题。
...
$$
另外一个思路是通过引入基准(baseline)$b$减小方差，这是因为实际计算中产生的总奖励并不能准确代表这个策略的好坏程度，比如一个不太好的样本策能得到一个正向的总奖励
$$
...
PG \approx\frac{1}{N}\sum_{i=1}^N[(\sum_{t=1}^Tlog\pi_\theta(a_{i,t}|s_{i,t}))(\sum_{t'=t}^Tr(a_{i,t'}, s_{i, t'})-b)]
$$
常见的基准值是均值
$$
b=\frac{1}{N}\sum_{i=1}^N r(a_i, s_i)
...
个人体会PG可以总结一个关键词是：抽样，通过抽样模拟目标函数，避免了遍历，由于抽样导致较大的方查
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTMwMTQ0MjI2OCwxNDUzNzk1ODkyLDE2NT
c2MjE3MzldfQ==
-->