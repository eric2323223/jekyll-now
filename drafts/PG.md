...
$$
其中，$s'$表示在当前状态$s$下使用动作$a$得到的下一个状态，$\gamma$表示对未来事件的折扣率，bellman方程描述了一个递归的计算方法，即$s$状态在使用$a$动作得到的总奖励等于当前的直接奖励$r$和下一步动作$a'$产生的总奖励的和。从这个方程可知，为了计算$s$状态下的最大奖励，需要求处其后所有状态的总奖励，因此由于需要保存所有状态的奖励，ji
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTE5MTQwNjk3MF19
-->