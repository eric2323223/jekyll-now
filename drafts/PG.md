...
$$
其中，$s'$表示在当前状态$s$下使用动作$a$得到的下一个状态$s'$，$\gamma$表示对未来事件的折扣率，bellman方程描述了一个递归的计算方法，即$s$状态在使用$a$动作得到的奖励等于当前的直接奖励$r$和下一步动作$a'$产生的奖励的和
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTI0NjkzMTQ3OV19
-->