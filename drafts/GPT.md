...
如下图（蓝色方框内）所示，GPT模型基本上就是Transformer模型的解码器部分，不同之处仅仅在于型的输入不同，Transformer解码器的输入是由编码器生成的**句向量**，解码器在解码时要先对已生成的输出进行自注意力计算，再进行编码器-解码器注意力计算  而GPT的输入是tokenized用户输入；其次是注意力机制的作用对象不同，Transformer中注意力关注对象是在句向量和解码器输出（？），而GPT
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA4ODY4OTk2MSwtMjAyMzUxNTUzNywtMT
A4MDQ5NzM5OSwxMDgzODAxNDAzLC0xMTE3MzkxMzM1LC0xMDgw
NDk3Mzk5LC0xNzY2MDUwNzkzLC0xODg0OTYxOTA4LDE2NjA0MD
Q3NTIsMzg1NDI5MjkzLDg5OTUwOTU0OCwxODYxMjYyNjcwLC0z
NzYxODY3OTQsNDU4NDQ5NTAwLC0xODAyOTQwMDIwLC0xODE5Mj
Y4ODE5LDE0MTIxNzE3MzMsODY3MDAxNTcxLDEwOTk0NTEyMDIs
LTE2Njc1NTY4NzZdfQ==
-->