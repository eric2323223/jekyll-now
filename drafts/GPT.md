...
- 模型深度对迁移学习的影响
GPT作者训练了多种不同深度（GPT解码器层数）的预训练模型，并在微调任务（RACE和MultiNLI）中对不同预训练模型的预测结果比较，如上图1中所示，可以观察到随着模型深度（层数）的增加，预测准确率也随之上升。这个结果说明了，增加解码器的层数可以让预训练模型携带更多可重用的特征，从而提高为
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzMDMwMjQyMzQsLTEzMjU4MjU0NjIsLT
