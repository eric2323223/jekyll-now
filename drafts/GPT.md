...
#### 编码层
- 词编码
GPT解码层使用一个长度为$h$ 768的浮点数向量来表示每个词，因此需要将tokenzied的词向量（长度为$v$)的长度转换为768位 ， 负责这个转换就是词编码。GPT中采用一个$v*h$转换矩阵 $W_{we}$将
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTMyMTI4NzUwMywtMTQzNzQwNDI4OSwtMT
