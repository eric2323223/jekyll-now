...
GPT的作者之前的一些研究中发现了大量的文本训练可以让基于RNN模型学习到一些可辩别的特征（discriminative features）。 为了验证这些学习到的特征具有通用性，并且可以应用于多种微调任务，作者设计了这样的迁移学习场景：首先对基于Transformer模型应用大量预料进行自监督训练， 然后将预训练模型进行微调后应用到多个微调任务场景（与预训练无关）测试中利用极少量的数据进行监督式训练，最后实验结果在多项公开测试zhong证明
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNzk3NjQzMzM2LDE0MTIxNzE3MzMsODY3MD
