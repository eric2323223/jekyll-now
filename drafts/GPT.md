...
如下图（蓝色方框内）所示，GPT模型基本上就是Transformer模型的解码器部分，不同之处仅有两点：首先是输入不同，Transformer解码器的输入是由编码器生成的**句向量**，而GPT的输入是用户的输入
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTc2MzMwNjM5NiwtMTc2NjA1MDc5MywtMT
