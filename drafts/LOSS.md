...
如何设计损失函数来引导模型选取合适的关键特征呢？最直接的方法就是计算不同个体特征向量的欧式距离，当两个个体之间的特征距离足够近的时候，就认为是同一个体，反之就是不同的个体。~~这就是对比误差函数（contrastive loss） **softmax loss?**
...
contrastive loss的缺点是用一个固定的$m$来衡量不同分类的距离，这显然具有很大的局限性。另外就是不同类型之间的距离。。。我们希望学习**关键指标，不同类指标距离大，相同类指标小**
Large margin softmax loss function 在论文中提出了新的思路，那就是将欧式距离转化为极坐标下的角度差，再通过$\|W_1\|\|x\|cos(m\theta_1) > \|W_2\|\|x\|cos(\theta_2)$强制分离不同类型，引导模型学习类间距离大，类内距离小的特征。
center loss 和Large margin的意图类似，在欧式坐标下
$$
L = L_{softmax} + \lambda L_{center} = -\sum_{i=1}^m log \frac{e^{y_i}}{\sum_j e^{y_j}} + \lambda \sum_{i=1}^m \|x_i-c_{y_i}\|^2_2
$$
...
- [The Loss Surfaces of Multilayer Networks](https://arxiv.org/pdf/1412.0233.pdf)
<!--stackedit_data:
eyJoaXN0b3J5IjpbODg4NDI4MjYwLC0xMzQ1MjA1ODgsMTYwNz
Q4Mjc4MCwtMTk3MjcxNjc2MiwxMzkwNjI5OTA3LC02OTkwOTA4
NzgsMTA4NDc0MjY3MSwtMTM5Njg5NjgxNywxODA3NDYzNTU1LD
E2Nzg2MTc2MzIsMTY3NDAxODUxMCwtMTI0ODMyOTE0MCwtODY2
MjUwMjk5LC0xMjQ4MzI5MTQwLC0yMTA5NDQwMzc1LDEwMDg5Nj
M3NSwtMTMyNjI4MDU5MiwtMTgxNjQwODQ0OSwtMjU5MzUyNjgs
LTE4MzY2Mjg1OTddfQ==
-->