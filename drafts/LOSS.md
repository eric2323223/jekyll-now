...
### 连续性
虽然GD要求loss function连续可导，但在实践中loss function可以存在不连续的点，这是因为计算是使用一组（batch）数据的loss均值进行求导，这样使得落在不可导的点上的概率显著降低，因此可以对0-1loss这样的不连续的函数使用GD来jinminibatch GD允许loss function在个别点不连续
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNzg5MzE4Mzc3LDIwMDAxMDk3MDcsLTY2Nz
